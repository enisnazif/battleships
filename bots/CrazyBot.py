    ...
    def get_ship_placements(self, ships: List[Ship]) -> List[Tuple[Ship, Point, Orientation]]:
        ...
        while True:
            ...
            for ship in ships:
                ...
                ship_len = len(ship)
                if random_orientation == Orientation.Vertical:
                    allowed_fields = set()
                    for x in range(10):
                        for y in range(10 - ship_len):
                            allowed_fields.add((x, y))
                else:
                    allowed_fields = set()
                    for x in range(10 - ship_len):
                        for y in range(10 ):
                            allowed_fields.add((x, y))
                allowed_fields = allowed_fields - invalid_fields
                placement = random.choice(list(allowed_fields))
                placements.append((ship, placement, random_orientation))
                for point in ship.get_points(placement, random_orientation):
                    for i in range(-1, 2, 1):
                        for j in range(-1, 2, 1):
                            invalid_fields.add((point.y + i, point.x + j))
